...
- Learn about `httr2`, a modern intuitive package to communicate with API
- Discuss some examples:
  - a simple first API: The Guardian
  - UK Parliament API
  - APSA 2023 "API"
  - ICA 2023
...
- In modern software development they are used extensively when:
  ...
  - when the inner workings of a software should be obscured, but the its functionality is offered for customization
  - when a graphic user interface would be inconvenient at scale
- Several important types (SOAP, GraphQL, etc.), but we will focus on REST (???) APIs
...
- an API **methods**: `GET`, `POST`, `PUT`, `DELETE`, etc. (only `GET` and sometimes `POST` are for us important)
...
- rewrite of the `httr2` which is the de-factor default to develop API packages in `R`
...
- tidyverse programming principles
  ...
  - makes wrapping an API in a few function or a package straightforward
...
Luckily, we know how to extract text from theat :)
...
- think of it as a general non-R-specific `httr2` (in fact, `httr2` just wrap `cURL`)
...
- The UK parliament APIs handle pagination through two parameters:
...
- There is a tool in `R` which opens a small server on your computer that can diplay OpenAPI specifications in the swagger format
